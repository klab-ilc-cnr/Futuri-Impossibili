...
export async function GET(request: Request) {
  try {
    ...
    const canonicalHeaders: Record<string, string> = { Accept: "text/plain" };
    if (authorization) canonicalHeaders.Authorization = authorization;
    const entries = await Promise.all(fileIds.map(async (fileId) => {
      const response = await fetch(
        ...
        { headers: canonicalHeaders, cache: "no-store" },
      );
      return [fileId, response.ok ? await response.text() : ""] as const;
    }));
