...
const wordType = "http://www.w3.org/ns/lemon/ontolex#Word";

export async function GET() {
  try {
    const parameters = new URLSearchParams({ type: wordType });
    const response = await fetch(
      `${lexoServerUrl}/service/lexica/it/entries?${parameters.toString()}`,
      {
