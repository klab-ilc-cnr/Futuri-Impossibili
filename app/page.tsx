...
export default function Home() {
  ...
  async function verifyWorkspacePassword(event: React.FormEvent) {
    ...
    try {
      ...
      const digestHex = subtle
        ? await (async () => {
            const digest = await subtle.digest("SHA-256", new TextEncoder().encode(passwordValue));
            return Array.from(new Uint8Array(digest))
              .map((byte) => byte.toString(16).padStart(2, "0"))
              .join("");
          })()
        : sha256Hex(passwordValue);
      if (digestHex === workspacePasswordHash) {
      ...
      setPasswordError(dictionaries[getLangSnapshot()].modals.passwordUnavailable);
      setPasswordValue("");
      passwordModalRef.current?.querySelector<HTMLInputElement>("input[type=password]")?.focus();
    } finally {
